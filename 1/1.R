...
s2 <- s1[s1$repository_language != "null", ]

library("ggplot2")

# Plot the pushes over time
ggplot(data=s2) + aes(x=month, y=pushes, group=repository_language, color=repository_language) + geom_line(aes(group=repository_language)) + geom_point()
